...
def filter_tus_by_metadata(tu_data, field_name, field_value):
    ...
    for tu in tu_data:
        if field_name in tu:
            ...
            tu_field_value = [custom_value for custom_name, custom_value in tu['CustomMetas'].items() if custom_name == field_name]
...
def save_tus_to_xlsx(tm_guid):
    ...
    for custom_meta in tu_data[1].get('CustomMetas', list()):
        ...
        tu_df_cols.append(custom_name) if custom_name else None
    tu_df = pd.DataFrame(columns=tu_df_cols)
    row_id = 0
    for tu in tu_data:
        tu_df.loc[row_id] = np.nan
        for field_name, field_value in tu.items():
            if field_name in ('SourceSegment', 'TargetSegment'):
                field_value = re.sub(r'<seg>(.+?)<\/seg>', r'\g<1>', field_value)
            if field_name == 'CustomMetas':
                for custom_fields in field_value:
                    custom_name = custom_fields.get('Name')
                    custom_value = custom_fields.get('Value')
                    tu_df.loc[row_id][custom_name] = custom_value
            else:
...
def add_tu(tm_guid, source, target, client=str(), domain=str(), project=str(), subject=str(), custom_metadata=dict()):
    ...
    tu_data['Client'] = f"{client}" if client else None
    ...
    if custom_metadata:
        ...
        for custom_name, custom_value in custom_metadata.items():
            tu_data["CustomMetas"].append({"Name": custom_name, "Value": custom_name})
    token = get_auth_token()
