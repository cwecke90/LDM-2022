...
def get_guid_by_tm_name(tm_data, name_start='', name_end=''):
    ...
    for tm in tm_data:
        ...
        tm_friendly_name = tm.get('FriendlyName')
        if not tm_friendly_name.endswith(name_end) and name_end:
            continue
        if not tm_friendly_name.startswith(name_start) and name_start:
            continue
        tms[tm_guid] = tm_friendly_name
    return tms


tm_data = list_tms()
print(get_guid_by_tm_name(tm_data, name_end='_CW'))
