...
def pre_process_chatbot_export(original_df, src_lang="en", trg_lang="jp"):
    ...
    # applied!
    formatted_df = pd.DataFrame(columns=original_df.columns)
    for index, row in original_df.iterrows():
        row_lang = row['Language']
        if row_lang == src_lang:
            row['Language'] = trg_lang
            formatted_df = formatted_df.append(row)
    return formatted_df
...
def post_process_chatbot_export(original_df, translated_df):
    ...
    """
    original_df = original_df.append(translated_df)
    original_df.sort_values(['Intent ID'])
    return original_df
    ...
    for worksheet in workbook:
        ...
        for cell in header_row:
            if cell.value == column_name:
                ...
                target_col = get_column_letter(target_col)
        if target_col:
