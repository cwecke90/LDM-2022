...
# tb_entries = tb_lookup(guid, 'car', lang='eng-US', term_condition="startswith")
    ...
    for entry in tb_entries:
        for lang in entry.get('Languages', list()):
            for term in lang.get('TermItems', list()):
                terms.append(term.get('Text', ''))
    for tu in tu_data:
        src_seg = tu.get('SourceSegment', '')
        trg_seg = tu.get('TargetSegment', '')
        for term in terms:
            if term in src_seg or term in trg_seg:
                if tu not in tus_w_terms:
                    tus_w_terms.append(tu)
    return tus_w_terms


# tb_guid = 'eda46967-44e9-4967-b1db-47b5f2910bcc'
